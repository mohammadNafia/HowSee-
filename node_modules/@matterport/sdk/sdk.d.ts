...
export declare namespace Camera {
	...
	type LookAtOptions = {
		/**
		 * Target view mode. If not specified, stays in current mode. Valid values include Dollhouse, Floorplan, and Inside.
		 */
		mode?: Mode.Mode;
		/**
		 * Type of transition animation. Not valid for Floorplan mode.
		 */
		transition?: Camera.TransitionType;
		/**
		 * Camera offset from the target position. The camera will be positioned at (position + offset) looking toward position.
		 */
		offset?: Vector3;
	};
	/**
...
export interface Camera {
	 ...
	 * @experimental
	 *
	 * Moves the camera to look at a specific 3D world position.
	 * Works in both dollhouse and inside (panorama) view modes.
	 *
	 * ```
	 * // Look at a position
	 ...
	 * ```
	 *
	 * @param {Vector3} position The 3D world position to look at.
	 * @param {Camera.LookAtOptions} options Optional parameters for the camera movement.
	 * @returns {Promise<void>} A Promise that resolves when the camera movement is complete.
	 *
	 * **Errors**
	 * * Fails if position coordinates are invalid (NaN or not numbers).
	 * * Fails if offset coordinates are invalid when provided.
	 */
	lookAt(position: Vector3, options?: Camera.LookAtOptions): Promise<void>;
	/**
...
export interface Measurements {
	 ...
	 * ```
	 */
	toggleMode(activate: boolean, options?: {
...
export declare namespace Tag {
	enum AttachmentType {
		...
		/** @hidden @internal Generic application files. Not currently functional for external attachments. */
		APPLICATION = "tag.attachment.application",
		/** @hidden @internal Audio files (e.g., MP3, WAV). Not currently functional for external attachments. */
		AUDIO = "tag.attachment.audio",
		/** Image files (e.g., JPEG, PNG, GIF). */
		IMAGE = "tag.attachment.image",
		/** @hidden @internal 3D model files (e.g., glTF, OBJ). Not currently functional for external attachments. */
		...
		/** @hidden @internal Plain text files. Not currently functional for external attachments. */
		...
		/**
		 * Custom HTML/script sandbox created via [[Tag.registerSandbox]].
		 * Cannot be used with [[Tag.registerAttachment]].
		 */
...
export declare namespace View {
	...
	interface View extends IObservable<View> {
		...
		/**
		 * Revert this view by removing all non-common, transient layers and removing any other object edits.
		 *
		 * ```ts
		 * await view.revert();
		 * ```
