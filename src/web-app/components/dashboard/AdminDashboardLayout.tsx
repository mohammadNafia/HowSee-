...
import { LayoutDashboard, Users, Home, BarChart3, Settings, LogOut, Menu, Globe } from 'lucide-react';
...
export function AdminDashboardLayout({ children }: AdminDashboardLayoutProps) {
  ...
  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, path: '/constructor/admin/dashboard' },
    { id: 'users', label: 'Users', icon: Users, path: '/constructor/admin/users' },
    { id: 'properties', label: 'Properties', icon: Home, path: '/constructor/admin/properties' },
    { id: 'analytics', label: 'Analytics', icon: BarChart3, path: '/constructor/admin/analytics' },
    { id: 'settings', label: 'Settings', icon: Settings, path: '/constructor/admin/settings' }
  ];

  const currentPage = menuItems.find(item => location.pathname === item.path)?.id || 'dashboard';
        ...
        <div className="flex flex-col h-full p-6">
          <div className="px-4 py-8 flex items-center gap-4">
            ...
            <div>
              ...
              <span className="text-xs font-semibold text-[#576856] uppercase tracking-wider">Admin Portal</span>
            </div>
          </div>
          ...
          <div className="mt-auto pt-6 border-t border-gray-100 flex items-center justify-between gap-1.5 sm:gap-2">
            <div className="flex items-center gap-2 sm:gap-3 px-2 py-1.5 sm:px-3 sm:py-2 bg-[#f4f5f2] rounded-2xl border border-gray-100 flex-1 min-w-0">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white flex items-center justify-center border border-gray-100 shadow-sm shrink-0">
                <span className="text-[#576856] font-bold text-[10px] sm:text-xs">A</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-[11px] sm:text-[13px] text-gray-900 truncate">Admin</p>
                <p className="text-[8px] sm:text-[9px] font-bold text-[#576856] tracking-tight truncate lowercase opacity-60">admin@howsee.com</p>
              </div>
            </div>
            <button 
